<template>
  <div class="relative inline-flex">
    <button
      ref="trigger"
      class="w-8 h-8 flex items-center justify-center hover:bg-gray-100 lg:hover:bg-gray-200 dark:hover:bg-zinc-700/50 dark:lg:hover:bg-zinc-800 rounded-full"
      :class="{ 'bg-gray-200 dark:bg-zinc-800': dropdownOpen }"
      aria-haspopup="true"
    ...
    >
      <div v-show="dropdownOpen" class="origin-top-right z-10 absolute top-full min-w-44 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-gray-700/60 py-1.5 rounded-lg shadow-lg overflow-hidden mt-1"  :class="align === 'right' ? 'right-0' : 'left-0'">
        ...
        >
          <li>
            <router-link class="font-medium text-sm text-amber-500 hover:text-amber-600 dark:hover:text-amber-400 flex items-center py-1 px-3" to="#0" @click="dropdownOpen = false">
              <svg class="w-3 h-3 fill-current text-amber-500 shrink-0 mr-2" viewBox="0 0 12 12">
