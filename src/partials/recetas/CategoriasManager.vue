<template>
  <div class="space-y-6">
      ...
      >
        <div class="flex items-center justify-between mb-3">
          ...
          <div class="flex space-x-2">
            ...
            <button
              @click="solicitarEliminacion(categoria)"
    ...
    <div
      ...
      class="fixed inset-0 backdrop-blur-sm bg-black/60 flex items-center justify-center z-50"
      @click.self="cerrarConfirmacion"
    >
      <div class="bg-zinc-900/95 border border-red-500/40 rounded-lg p-6 w-full max-w-md mx-4">
        <div class="flex items-center mb-4">
          <div class="w-10 h-10 rounded-full bg-red-500/20 text-red-400 flex items-center justify-center mr-3">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <div>
            <h3 class="text-lg font-semibold text-white">Eliminar categoría</h3>
            <p class="text-sm text-gray-400">Esta acción no se puede deshacer.</p>
          </div>
        </div>
        <p class="text-gray-200 mb-6">
          ¿Seguro que deseas eliminar la categoría
          <span class="font-semibold text-white">{{ categoriaAEliminar?.nombre }}</span>?
        </p>
        <div class="flex justify-end space-x-3">
          ...
          <button
            ...
            class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-400 transition-colors font-medium"
          >
            Eliminar
          </button>
        </div>
