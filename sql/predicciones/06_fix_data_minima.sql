...
BEGIN
    ...
    FOR i IN 1..3 LOOP
        v_mes_actual := p_fecha_inicio - (i || ' month')::INTERVAL;
...
CREATE OR REPLACE FUNCTION obtener_top_insumos(p_limite INT DEFAULT 10)
RETURNS TABLE (
    insumo_id UUID,
...
BEGIN
    ...
    SELECT
        ...
        COUNT(DISTINCT DATE_TRUNC('month', mi.fecha_movimiento)) as meses_con_datos
    FROM movimientos_inventario mi
    INNER JOIN lotes_insumos li ON mi.lote_id = li.id
    WHERE mi.tipo IN ('salida', 'consumo', 'ajuste_salida')
      AND mi.fecha_movimiento >= CURRENT_DATE - INTERVAL '3 months'
    GROUP BY li.insumo_id
    HAVING SUM(mi.cantidad) > 0 -- Solo insumos con consumo
...
-- =====================================================
-- 5. ACTUALIZAR: Función de generación de predicciones de insumos
-- =====================================================
...
DECLARE
    v_fecha_inicio DATE := DATE_TRUNC('month', CURRENT_DATE + INTERVAL '1 month');
    v_fecha_fin DATE := (DATE_TRUNC('month', CURRENT_DATE + INTERVAL '1 month') + INTERVAL '1 month - 1 day')::DATE;
    v_insumo RECORD;
    v_pmp NUMERIC;
...
-- Ver si las funciones se actualizaron correctamente
